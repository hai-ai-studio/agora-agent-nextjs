...
- **`agora-agent-uikit`** exposes the `IMessageListItem` + `AgentVisualizerState` type shapes consumed by `src/features/conversation/lib/transcript.ts` (helpers) and `src/features/conversation/lib/visualizer-state.ts` (`mapAgentVisualizerState`). The quickstart's main UI does not render uikit runtime components — the Aria view layer in `src/features/conversation/components/` (`Transcript`, `Waveform`, `Persona`, `Controls`) is used instead. If you prefer the turnkey uikit experience, `ConvoTextStream` and `AgentVisualizer` remain installed and can be dropped in.
...
2. **The Face (`src/features/conversation/components/Transcript.tsx` + `Waveform.tsx`)**: The Aria view layer consumes the derived `messageList` / `currentInProgressMessage` and renders the side-panel transcript + two-row bar waveform. The uikit's `ConvoTextStream` + `AgentVisualizer` remain available as drop-in alternatives.
3. **The Conductor (`src/features/conversation/components/ConversationShell.tsx` + `src/features/conversation/hooks/useAgoraVoiceAI.ts`)**: Handles the Agora RTC/RTM connection, initializes `AgoraVoiceAI` (inside `useAgoraVoiceAI`), subscribes to transcript and state events, remaps UIDs, adapts the transcript shape, and passes data down to the UI layer.
...
flowchart LR
    ...
    B -- TRANSCRIPT_UPDATED --> C(ConversationShell State)
    C -- messageList + currentInProgressMessage --> D(ConvoTextStream UI)
```

Raw data arrives from the agent over RTM. `AgoraVoiceAI` processes it and emits transcript events. `ConversationShell` remaps UIDs and updates state. `ConvoTextStream` renders the result.
...
graph LR
    ...
    C --> D(ConversationShell Maps & Updates State);
    ...
    subgraph ConvoComp [ConversationShell]
        D
    end
...
4. **State Updates**: `ConversationShell` remaps UIDs, separates completed and in-progress turns, updates React state.
...
## Wiring Up `AgoraVoiceAI` in `useAgoraVoiceAI`

The quickstart extracts this into a dedicated hook at `src/features/conversation/hooks/useAgoraVoiceAI.ts`. Initialize `AgoraVoiceAI` inside a `useEffect` that fires only after `joinSuccess` — this ensures the RTC client is connected and avoids React StrictMode double-initialization. In the shipped code the effect is gated on `enabled: isReady && joinSuccess`; the snippet below uses raw `joinSuccess` for readability.
...
// Inside useAgoraVoiceAI (or ConversationShell):
...
The RTM client must be created, logged in, and subscribed to the channel **before** `ConversationShell` mounts — the toolkit needs a fully connected RTM client at init time.

Create it in `useAgoraSession` (which `LandingPage` composes) as part of the session setup:
...
Then pass it as a prop to `ConversationShell`. On end conversation, call `rtmClient.logout()` in `LandingPage` — RTM is owned by the same scope that created it.
...
Text streaming requires the agent to be started with RTM enabled. In `src/app/api/invite-agent/route.ts`:
