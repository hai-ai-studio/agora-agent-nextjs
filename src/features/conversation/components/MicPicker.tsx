...
export function MicPicker({ localMicrophoneTrack }: MicPickerProps) {
  ...
  return (
    <div className="mic-picker relative inline-flex">
      <button
        type="button"
        className="flex h-11 w-5 cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-ink-3 transition-colors duration-150 hover:bg-black/[0.06] hover:text-ink"
        onClick={() => setOpen((o) => !o)}
        aria-haspopup="menu"
        aria-expanded={open}
      ...
      {open && (
        <motion.div
          ...
          className="absolute bottom-[calc(100%+12px)] left-0 z-[5] max-h-60 w-[260px] max-w-[calc(100vw-32px)] overflow-y-auto rounded-[18px] border border-line bg-white/95 p-2 shadow-[0_20px_60px_rgba(0,0,0,0.12)] backdrop-blur-[24px] supports-[backdrop-filter]:bg-white/95 max-[640px]:left-auto max-[640px]:right-0"
        >
          ...
          {devices.map((device) => (
            <button
              key={device.deviceId}
              type="button"
              className={`flex w-full cursor-pointer items-center justify-between gap-2.5 rounded-[10px] border-none bg-transparent px-2.5 py-2 text-left font-sans text-[13px] transition-colors duration-[120ms] hover:bg-bg-2 ${
